...
var _ = Describe("HorizontalScaling OpsRequest", func() {
	...
	Context("Test OpsRequest", func() {
		...
		It("test offline the specified pod but it is not online with policy ignore", func() {
			By("init operations resources with CLusterDefinition/ClusterVersion/Hybrid components Cluster/consensus Pods")
			opsRes, _, _ := initOperationsResources(clusterDefinitionName, clusterVersionName, clusterName)
			...
			Eventually(testapps.GetOpsRequestPhase(&testCtx, client.ObjectKeyFromObject(opsRes.OpsRequest))).Should(Equal(appsv1alpha1.OpsFailedPhase), fmt.Sprintf("info: %v", opsRes.OpsRequest))
		})
		...
		It("test offline and online two pods in the same time with ignore policy", func() {
			By("init operations resources with CLusterDefinition/ClusterVersion/Hybrid components Cluster/consensus Pods")
			opsRes, _, _ := initOperationsResources(clusterDefinitionName, clusterVersionName, clusterName)
			// opsRes1, _, _ := initOperationsResources(clusterDefinitionName+"1", clusterVersionName, clusterName)
			testapps.MockInstanceSetComponent(&testCtx, clusterName, consensusComp)
			reqCtx := intctrlutil.RequestCtx{Ctx: ctx}
			testPodNameScaleIn := fmt.Sprintf("%s-%s-1", clusterName, consensusComp)
			testPodNameScaleOut := fmt.Sprintf("%s-%s-2", clusterName, consensusComp)
			By("offline two pod with same pod name, expect only scalein success")
			_ = createOpsAndToCreatingPhase(reqCtx, opsRes, appsv1alpha1.HorizontalScaling{
				ScaleIn: &appsv1alpha1.ScaleIn{
					OnlineInstancesToOffline: []string{testPodNameScaleIn},
				},
				ScaleOut: &appsv1alpha1.ScaleOut{
					OfflineInstancesToOnline: []string{testPodNameScaleOut},
				},
			}, constant.HscaleValidatePolicyIgnore)
			Eventually(testapps.GetOpsRequestPhase(&testCtx, client.ObjectKeyFromObject(opsRes.OpsRequest))).Should(Equal(appsv1alpha1.OpsCreatingPhase))
			Expect(opsRes.Cluster.Spec.GetComponentByName(consensusComp).Replicas).Should(BeEquivalentTo(2))
			...
			Expect(onlineToOfflineInstances).Should(Equal([]string{testPodNameScaleIn}), fmt.Sprintf("info: %v", opsRes.OpsRequest))
			Expect(offlineToOnlineInstances).Should(Equal([]string{testPodNameScaleOut}), fmt.Sprintf("info: %v", opsRes.OpsRequest))
			// expect for opsRequest phase is Succeed after pods has been scaled and component phase is Running
			By("expect for opsRequest phase is Succeed after pods has been scaled and component phase is Running")
			// mock consensus component is Running
			mockConsensusCompToRunning(opsRes)
			_, err := GetOpsManager().Reconcile(reqCtx, k8sClient, opsRes)
			Expect(err).ShouldNot(HaveOccurred())
			Expect(opsRes.OpsRequest.Status.Progress).Should(Equal("2/2"), fmt.Sprintf("info: %v", opsRes.OpsRequest))
			Expect(opsRes.OpsRequest.Status.Phase).Should(Equal(appsv1alpha1.OpsSucceedPhase))
		})
